...
if Menu_Options == 'Data Visualization':
    ...
    if Visualization_options == 'Confusion Matrix':
        visual_models_cm = st.sidebar.selectbox('Prediction using Models',('BernoulliNB','Logistic Regression','GradientBoostingClassifier','LinearSVC', 'CatBoostClassifier'))
        ...
        if plot_cm:
            ...
            if visual_models_cm == 'CatBoostClassifier':
                ModelName = model_list[0]
                backend.confusion_matrix_func(ModelName,X_train, X_test, y_test, y_train)
...
if Menu_Options == 'Prediction':
    ...
    Models_toggle = st.sidebar.selectbox('Prediction using Models',('BernoulliNB','Logistic Regression','GradientBoostingClassifier','LinearSVC', 'Distilbert Pipeline', 'CatBoostClassifier'))
...
# CatBoostClassifier Model
    if Models_toggle == backend.models[0]:
        ...
        lm = st.slider('Alpha',0, 10, (1))
        if predict_sentiment:
            data,result,train_AUC,test_AUC = backend.predict_model(CBmodel,user_input,X_train,X_test,y_train, y_test,vectorizer,alpha=lm)
